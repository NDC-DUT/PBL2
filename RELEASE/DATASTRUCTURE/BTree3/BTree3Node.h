...
public:
    ...
    BTree3Node(const DATA_T &&leftValue)
        : leftValue{std::move(leftValue)}, isFull{false}, midChild{nullptr}, leftChild{nullptr}, rightChild{nullptr}
    {
    }
    ...
    BTree3Node(const DATA_T &&leftValue, BTree3Node *leftChild, BTree3Node *midChild)
        : leftValue{std::move(leftValue)}, isFull{false}, leftChild{leftChild}, midChild{midChild}, rightChild{nullptr}
    {
    }
    ~BTree3Node() = default;
};